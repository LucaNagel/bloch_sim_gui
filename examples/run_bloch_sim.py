...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run Bloch simulation from the command line."
    )
    p.add_argument(
        "--flip", type=float, default=90.0, help="Flip angle in degrees (default: 90)"
    )
    p.add_argument(
        "--duration_ms",
        type=float,
        default=1.0,
        help="RF pulse duration in ms (default: 1.0)",
    )
    p.add_argument(
        "--nfreq",
        type=int,
        default=1,
        help="Number of off-resonance frequencies (default: 1)",
    )
    p.add_argument(
        "--freq_span",
        type=float,
    ...
    p.add_argument(
        "--npos", type=int, default=1, help="Number of positions along x (default: 1)"
    )
    p.add_argument(
        "--pos_span_cm",
        type=float,
    ...
    p.add_argument(
        "--use_parallel",
        action="store_true",
        help="Enable parallel simulation when many spins are present.",
    )
    return p.parse_args()
...
def main():
    ...
    b1, time = design_rf_pulse(
        "rect", duration=duration_s, flip_angle=args.flip, npoints=npoints
    )
    ...
    print(
        f"RF points: {len(b1)}, dt ~ {np.mean(np.diff(time))*1e6:.3f} us, duration: {duration_s*1e3:.3f} ms"
    )
    print(f"Positions: {positions.shape}, Frequencies: {frequencies}")
    print(
        f"mx shape: {np.shape(result['mx'])}, my shape: {np.shape(result['my'])}, mz shape: {np.shape(result['mz'])}"
    )
    print(
        f"signal shape: {np.shape(result['signal'])}, time shape: {np.shape(result['time'])}"
    )
    ...
    for pi in range(mx.shape[0]):
        for fi in range(mx.shape[1]):
            ...
            print(
                f"  pos {pi:02d}, freq {fi:02d}: mx={mx[pi, fi]: .4f}, my={my[pi, fi]: .4f}, mz={mz[pi, fi]: .4f}, |Mxy|={mxy: .4f}"
            )
