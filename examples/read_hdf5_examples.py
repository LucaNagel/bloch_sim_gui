...
def plot_magnetization_evolution(data, position_idx=0, freq_idx=0):
    ...
    plt.suptitle(
        f"Magnetization Evolution - {tissue_name}\n"
...
def plot_signal(data, position_idx=0, freq_idx=0):
    ...
    plt.suptitle(
        f"MRI Signal - Position {position_idx}, Frequency {freq_idx}",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def plot_spatial_profile(data, freq_idx=0, time_idx=-1):
    ...
    if data["mz"].ndim == 3:  # Time-resolved
        ...
        mxy = np.sqrt(
            data["mx"][time_idx, :, freq_idx] ** 2
            + data["my"][time_idx, :, freq_idx] ** 2
        )
    elif data["mz"].ndim == 2:  # Endpoint
        mz = data["mz"][:, freq_idx]
        mxy = np.sqrt(data["mx"][:, freq_idx] ** 2 + data["my"][:, freq_idx] ** 2)
    else:
    ...
    plt.suptitle(
        f"Spatial Profile - Frequency: {freq:.1f} Hz, Time: {time_ms:.3f} ms",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def quick_analysis(data):
    ...
    print(
        f"  Position range: {data['positions'][:, 2].min()*100:.3f} to {data['positions'][:, 2].max()*100:.3f} cm"
    )
    ...
    print(
        f"  Frequency range: {data['frequencies'].min():.1f} to {data['frequencies'].max():.1f} Hz"
    )
    ...
    if data["mx"].ndim == 3:
        ...
        if peak_idx < len(time) - 10:
            ...
            try:
                ...
                print(
                    f"  Difference: {abs(popt[1] - data['tissue']['t2'])*1000:.2f} ms"
                )
            except:
