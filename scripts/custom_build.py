...
def build_extension():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            extensions = glob.glob("blochsimulator_cy*.so") + glob.glob(
                "blochsimulator_cy*.pyd"
            )
            if extensions:
...
def test_import():
    ...
    try:
        ...
        functions = [
            name for name in dir(blochsimulator_cy) if not name.startswith("_")
        ]
...
def main():
    ...
    print(
        "3. Run examples: python -c 'from blochsimulator import example_fid; example_fid()'"
    )
